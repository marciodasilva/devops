...
public class PasswordResetToken implements Serializable {
    ...
    private static final int DEFAUL_TOKEN_LENGTH_IN_MINUTES = 120;
    ...
    public PasswordResetToken(String token, User user, LocalDateTime creationDateTime, int expirationInMinutes){
        if ((null == token) || (null == user) || (null == creationDateTime)) {
            throw new IllegalArgumentException("token, user and creationDateTime can't be null");
        }
        if (expirationInMinutes == 0){
            LOG.warn("The token expiration legth in minutes is Zero, Assigning the default vulue {} ",DEFAUL_TOKEN_LENGTH_IN_MINUTES);
            expirationInMinutes = DEFAUL_TOKEN_LENGTH_IN_MINUTES;
        }
        this.token = token;
        this.user = user;
